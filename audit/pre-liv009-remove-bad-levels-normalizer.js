...
function createLiv009StageboxInput(layer, key, point, falseTarget) {
  ...
  function sfLiv009NormalizeRoutes(){
    const spec = LEVELS && LEVELS["LIV-009"];
    if(!spec) return;

    spec.sourceOrder = [];

    spec.generatedJackKeys = [
      "stagebox-input-1",
      "stagebox-input-2",
      "stagebox-input-3",
      "stagebox-input-4",
    ...
    spec.validRoutes = [
      ...
      },
      {
        ...
        from: "overhead-left-crash",
        to: "stagebox-input-7",
        checklist: "OH Left → Stage Box Input 7",
        stereoGroup: "drum-overheads",
        stereoSide: "left"
      },
      {
        ...
        from: "overhead-right-ride",
        to: "stagebox-input-8",
        checklist: "OH Right → Stage Box Input 8",
        stereoGroup: "drum-overheads",
        stereoSide: "right"
      }
    ];

    console.log("[Signal Flow] LIV-009 routes normalized:", spec.validRoutes.map(r => r.key));
  }


function renderLiv009DrumStageInputs(surface, adapter) {
    sfLiv009NormalizeRoutes();

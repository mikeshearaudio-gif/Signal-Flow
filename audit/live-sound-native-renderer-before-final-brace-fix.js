...
function renderLiv009DrumStageInputs(surface, adapter) {
    ...
    [
      ["lead-vocal-mic", "Lead Vocal Mic", 65, 280, 133, 158],
      ["liv021-stagebox-input-1", "Stage Box Input 1", 34, 226, 32, 42],
      ["liv021-ch1-insert-send", "Channel 1 Insert Send", 254, 49, 32, 42],
      ["liv021-compressor-input", "Vocal Compressor Input", 337, 242, 32, 42],
      ["liv021-compressor-output", "Vocal Compressor Output", 402, 242, 32, 42],
      ["liv021-ch1-insert-return", "Channel 1 Insert Return", 254, 76, 32, 42],
      ["liv021-aux-1-output", "Auxiliary 1 Output", 423, 56, 32, 42],
      ["liv021-vocal-wedge-amp-input", "Vocal Wedge Amp Input", 569, 417, 32, 42],
      ["liv021-vocal-wedge-amp-output", "Vocal Wedge Amp Output", 754, 417, 32, 42],
      ["liv021-vocal-wedge-input", "Vocal Wedge Input", 275, 307, 222, 117],
      ["liv021-main-left-output", "Main Left Output", 729, 57, 32, 42],
      ["liv021-main-right-output", "Main Right Output", 765, 57, 32, 42],
      ["liv021-system-processor-left-input", "System Processor Left Input", 568, 229, 32, 42],
      ["liv021-system-processor-right-input", "System Processor Right Input", 599, 229, 32, 42],
      ["liv021-system-processor-left-output", "System Processor Left Output", 678, 219, 32, 67],
      ["liv021-system-processor-right-output", "System Processor Right Output", 709, 219, 32, 67],
      ["liv021-main-amp-left-input", "Main Amp Left Input", 574, 322, 34, 42],
      ["liv021-main-amp-right-input", "Main Amp Right Input", 606, 322, 32, 42]
    ].forEach(function(h) {
      addLiv021DevNode(h[0], h[1], h[2], h[3], h[4], h[5]);
    });
...
function renderLiv020MainPaAndIem(surface, adapter) {
  ...
  function renderLiv021VocalInsertDevScaffold(surface, adapter) {
    ...
    function addGear(id, src, x, y, w, label, h) {
      const img = document.createElement("img");
    ...
    node._sfNativeNode = nativeNode;

    node.addEventListener("pointerdown", function () {
      window.sfLiv021DragStartNode = nativeNode;
    });

    node.addEventListener("pointerup", function (event) {
      const fromNode = window.sfLiv021DragStartNode;
      window.sfLiv021DragStartNode = null;

      const targetEl = document.elementFromPoint(event.clientX, event.clientY);
      const toButton = targetEl && targetEl.closest && targetEl.closest("[data-node-key]");
      const toNode = toButton && toButton._sfNativeNode;

      if (fromNode && toNode && fromNode.key !== toNode.key) {
        addRoute(layer, fromNode, toNode);
        clearSelection();
        event.preventDefault();
        event.stopPropagation();
      }

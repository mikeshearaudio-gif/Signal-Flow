...
backup = LAUNCH.with_suffix('.html.pre-build-room-v6r232.bak')
...
# Retire the legacy Mic Locker label at source/template level. The v6r232 renderer
# intercepts the old click path and opens the Equipment Locker instead.
...
css = '<link rel="stylesheet" href="/patch/sf-build-room-renderer.css?v=6r232">'
js = '<script src="/patch/sf-build-room-renderer.js?v=6r232"></script>'
block_head = '\n<!-- Signal Flow Build-a-Room consolidated renderer v6r232 -->\n' + css + '\n<!-- /Signal Flow Build-a-Room consolidated renderer v6r232 -->\n'
block_body = '\n<!-- Signal Flow Build-a-Room consolidated renderer v6r232 -->\n' + js + '\n<!-- /Signal Flow Build-a-Room consolidated renderer v6r232 -->\n'
if '</head>' not in html.lower() or '</body>' not in html.lower():
    raise SystemExit('Launch HTML is missing </head> or </body>')
html = re.sub(r'sf-build-room-v6r\d+-mount', 'sf-build-room-v6r232-mount', html)
...
out.write_text(json.dumps({'version':'6r232','shelves':shelves}, indent=2))
...
print('[OK] Installed Build-a-Room consolidated renderer v6r232')

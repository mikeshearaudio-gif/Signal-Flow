...
const board = readJson("data/live-sound/boards/liv026.json");
const normalized = readJson("data/live-sound/boards/normalized/liv026.normalized.json");
const routesEvidence = readJson("audit/liv026-preservation-snapshot/routes.json");
const groupsEvidence = readJson("audit/liv026-preservation-snapshot/stereo-groups.json");
const goodHitboxesEvidence = readJson("audit/liv026-preservation-snapshot/good-hitboxes.json");
const falseHitboxesEvidence = readJson("audit/liv026-preservation-snapshot/false-hitboxes.json");
const wrongRouteEvidence = readJson("audit/liv026-preservation-snapshot/wrong-route-behavior.json");
const lockedEvidence = readJson("audit/liv026-preservation-snapshot/locked-behavior.json");
...
assert(fs.existsSync(sourceManifestPath), "Missing source manifest: data/live-sound/boards/liv026.json");
assert(fs.existsSync(normalizedManifestPath), "Missing normalized manifest: data/live-sound/boards/normalized/liv026.normalized.json");

if (board && routesEvidence) {
  assert(board.levelId === "LIV-026", "Source manifest levelId must be LIV-026");
...
if (board && goodHitboxesEvidence && falseHitboxesEvidence) {
  ...
  assert(board.hitboxes.good.length === 31, `Source manifest must preserve 31 baked true hitboxes, found ${board.hitboxes.good.length}`);
  assert(board.hitboxes.false.length === 28, `Source manifest must preserve 28 false hitboxes, found ${board.hitboxes.false.length}`);
  assert(board.acceptance?.bakedTrueHitboxCount === 31, "Source manifest acceptance.bakedTrueHitboxCount must be 31");
  assert(board.acceptance?.requiredEndpointHitboxCount === 30, "Source manifest acceptance.requiredEndpointHitboxCount must be 30");
  assert(board.acceptance?.unusedBakedTrueHitboxCount === 1, "Source manifest acceptance.unusedBakedTrueHitboxCount must be 1");
  assert(board.acceptance?.falseHitboxCount === 28, "Source manifest acceptance.falseHitboxCount must be 28");
  ...
  assert(JSON.stringify(boardGoodIds) === JSON.stringify(snapshotGoodIds), "Source manifest baked true hitbox IDs must match snapshot");

  const unused = board.hitboxes.good.filter(hitbox => hitbox.preservationOnly === true || hitbox.completionCredit === false);
  assert(unused.length === 1, `Source manifest must keep exactly one unused baked true hitbox, found ${unused.length}`);
  assert(unused[0]?.id === "liv026-delay-processor-input-unused", `Unused baked true hitbox must be liv026-delay-processor-input-unused, found ${unused[0]?.id || "(none)"}`);
  assert(unused[0]?.hintable === false, "Unused baked true hitbox must not be hintable");
  assert(unused[0]?.completionCredit === false, "Unused baked true hitbox must not count toward completion");
  ...
  assert(!validEndpointKeys.has("liv026-delay-processor-input-unused"), "Unused baked true hitbox must not be a valid route endpoint");
...
if (board && wrongRouteEvidence) {
  ...
  assert(String(board.invalidRouteEvidence?.broadInvalidRouteRule?.rule || "").includes("liv026-"), "Invalid route evidence must record broad liv026-* behavior");
}
...
if (board && lockedEvidence) {
  ...
  assert(board.preservation?.unusedBakedTrueHitbox === "liv026-delay-processor-input-unused", "Source manifest must preserve unused baked true hitbox identity");
  assert(board.preservation?.lockedBehavior?.customScrollHostClass === "sf-live-native-liv026-scroll-host", "Source manifest must preserve custom scroll host reference");
  ...
  assert(board.preservation?.lockedBehavior?.falseHitboxLockEvidenceFound === true, "Source manifest must preserve false-hitbox lock evidence");
  assert(board.preservation?.lockedBehavior?.stackGuardEvidenceFound === true, "Source manifest must preserve stack guard evidence");
  assert(board.preservation?.lockedBehavior?.processorDecoCleanupEvidenceFound === true, "Source manifest must preserve processor-deco cleanup evidence");
}
...
if (normalized && board) {
  assert(normalized.levelId === "LIV-026", "Normalized manifest levelId must be LIV-026");
  ...
  assert(normalized.hitboxes?.good?.length === 31, "Normalized manifest hitboxes.good must contain 31 baked true hitboxes");
  assert(normalized.hitboxes?.false?.length === 28, "Normalized manifest hitboxes.false must contain 28 false hitboxes");
  assert(normalized.nodes?.validEndpointKeys?.length === 30, "Normalized manifest validEndpointKeys must contain 30 required endpoints");
  assert(!normalized.nodes?.validEndpointKeys?.includes("liv026-delay-processor-input-unused"), "Normalized manifest must not treat unused baked true hitbox as valid endpoint");
  assert(normalized.nodes?.falseTrapKeys?.length === 28, "Normalized manifest falseTrapKeys must contain 28 false hitboxes");
...
const summary = {
  ...
  counts: {
    ...
    bakedTrueHitboxes: board?.hitboxes?.good?.length ?? null,
    requiredEndpointHitboxes: normalized?.nodes?.validEndpointKeys?.length ?? null,
    falseHitboxes: board?.hitboxes?.false?.length ?? null,
    normalizedRoutes: normalized?.routes?.length ?? null
  },
  unusedBakedTrueHitbox: board?.preservation?.unusedBakedTrueHitbox ?? null,
  status: liveSoundMap?.levels?.["LIV-026"]?.status ?? null,
  failures
};

if (failures.length) {
  console.error("LIV-026 manifest parity check failed");
  console.error(JSON.stringify(summary, null, 2));
  process.exit(1);
}

console.log("LIV-026 manifest parity check passed");

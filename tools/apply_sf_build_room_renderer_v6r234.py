...
backup = LAUNCH.with_suffix('.html.pre-build-room-v6r234.bak')
...
css = '<link rel="stylesheet" href="/patch/sf-build-room-renderer.css?v=6r234" />'
js = '<script src="/patch/sf-build-room-renderer.js?v=6r234"></script>'
source_override = r'''
<!-- Signal Flow Build-a-Room Equipment Locker source override v6r234 -->
<script>
(function(){
...
<!-- /Signal Flow Build-a-Room Equipment Locker source override v6r234 -->
'''
block_head = '\n<!-- Signal Flow Build-a-Room consolidated renderer v6r234 -->\n' + css + '\n<!-- /Signal Flow Build-a-Room consolidated renderer v6r234 -->\n'
...
html = re.sub(r'sf-build-room-v6r\d+-mount', 'sf-build-room-v6r234-mount', html)
...
out.write_text(json.dumps({'version':'6r234','shelves':shelves}, indent=2))
...
if asset_path.exists():
    ...
    def ensure_item(item_id, name, category, aliases=(), paths=(), clear_if_no_paths=False):
        ...
        if not it:
            it = {'id':item_id, 'displayName':name, 'category':category, 'family':'gear', 'assetPaths':[], 'aliases':[], 'source':'v6r234-normalized'}
...
print('[OK] Installed Build-a-Room consolidated renderer v6r234')

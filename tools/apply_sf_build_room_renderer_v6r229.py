...
backup = LAUNCH.with_suffix('.html.pre-build-room-v6r229.bak')
...
# Remove accidental duplicate v6r229 injections if rerun.
html = re.sub(r'\n?\s*<!-- Signal Flow Build-a-Room consolidated renderer v6r229 -->[\s\S]*?<!-- /Signal Flow Build-a-Room consolidated renderer v6r229 -->\s*', '\n', html)

css = '<link rel="stylesheet" href="/patch/sf-build-room-renderer.css?v=6r229">'
js = '<script src="/patch/sf-build-room-renderer.js?v=6r229"></script>'
block_head = '\n<!-- Signal Flow Build-a-Room consolidated renderer v6r229 -->\n' + css + '\n<!-- /Signal Flow Build-a-Room consolidated renderer v6r229 -->\n'
block_body = '\n<!-- Signal Flow Build-a-Room consolidated renderer v6r229 -->\n' + js + '\n<!-- /Signal Flow Build-a-Room consolidated renderer v6r229 -->\n'
if '</head>' not in html.lower() or '</body>' not in html.lower():
    raise SystemExit('Launch HTML is missing </head> or </body>')
html = html.replace('sf-build-room-v6r227-mount', 'sf-build-room-v6r229-mount')
...
print('[OK] Installed Build-a-Room consolidated renderer v6r229')

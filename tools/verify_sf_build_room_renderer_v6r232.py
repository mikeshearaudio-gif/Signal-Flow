...
for ref in ['sf-build-room-renderer.css?v=6r232','sf-build-room-renderer.js?v=6r232']:
...
for bad in ['setInterval(installSplashLocker', 'cloneNode(true)', 'replaceWith(clone)', 'v6r228', 'v6r229', 'v6r230', 'v6r231', 'sf-br-splash-locker-overlay = document.createElement']:
    if bad in js: fail.append(f'Forbidden renderer pattern still present: {bad}')
if 'installed v6r232' not in js: fail.append('Renderer install log is not v6r232')
if fail:
    print('[FAIL] Build-a-Room v6r232 verification failed:')
    for f in fail: print(' -', f)
    sys.exit(1)
print('[PASS] Build-a-Room v6r232 single-owner install is clean.')

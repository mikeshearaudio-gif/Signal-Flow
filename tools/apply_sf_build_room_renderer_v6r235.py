...
backup = LAUNCH.with_suffix('.html.pre-build-room-v6r235.bak')
...
css = '<link rel="stylesheet" href="/patch/sf-build-room-renderer.css?v=6r235" />'
js = '<script src="/patch/sf-build-room-renderer.js?v=6r235"></script>'
source_override = r'''
<!-- Signal Flow Build-a-Room Equipment Locker source override v6r235 -->
<script>
(function(){
...
<!-- /Signal Flow Build-a-Room Equipment Locker source override v6r235 -->
'''
block_head = '\n<!-- Signal Flow Build-a-Room consolidated renderer v6r235 -->\n' + css + '\n<!-- /Signal Flow Build-a-Room consolidated renderer v6r235 -->\n'
...
html = re.sub(r'sf-build-room-v6r\d+-mount', 'sf-build-room-v6r235-mount', html)
...
out.write_text(json.dumps({'version':'6r235','shelves':shelves}, indent=2))
...
if asset_path.exists():
    ...
    def best(include, exclude=()):
        ...
        return ''
    def first_best(patterns):
        for include, exclude in patterns:
            hit = best(include, exclude)
            if hit: return hit
        return ''
    def ensure_item(item_id, name, category, aliases=(), paths=(), clear_if_no_paths=False):
        it = by_id.get(item_id)
        if not it:
            it = {'id':item_id, 'displayName':name, 'category':category, 'family':'gear', 'assetPaths':[], 'aliases':[], 'source':'v6r235-normalized'}
        ...
        elif clear_if_no_paths:
            it['assetPaths'] = []
        return it
    di = first_best([
        (('di-box',), ('diaphragm','goodstyle')),
        (('di_box',), ('diaphragm','goodstyle')),
        (('dibox',), ('diaphragm','goodstyle')),
        (('passive','di'), ('diaphragm','goodstyle')),
        (('direct','box'), ('diaphragm','goodstyle')),
        (('direct','injection'), ('diaphragm','goodstyle')),
    ])
    dyn = first_best([
        (('sm58',), ('locked',)),
        (('sm57',), ('locked',)),
        (('dynamic','mic'), ('condenser','box','locked')),
        (('handheld','mic'), ('condenser','box','locked')),
        (('dynamic','microphone'), ('condenser','box','locked')),
    ])
    ensure_item('passive-di-box','Passive DI box','Cables / DI', ['DI box','direct box','direct injection box'], [di], clear_if_no_paths=True)
    ensure_item('dynamic-cardioid-mic','Dynamic cardioid mic','Mics', ['Handheld dynamic mic','SM58 dynamic mic','SM57 dynamic mic'], [dyn], clear_if_no_paths=True)
    ensure_item('handheld-dynamic-cardioid-mic','Handheld dynamic cardioid mic','Mics', ['Dynamic cardioid mic','SM58 dynamic mic'], [dyn], clear_if_no_paths=True)
...
print('[OK] Installed Build-a-Room consolidated renderer v6r235')

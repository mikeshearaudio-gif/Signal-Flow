...
backup = LAUNCH.with_suffix('.html.pre-build-room-v6r231.bak')
...
css = '<link rel="stylesheet" href="/patch/sf-build-room-renderer.css?v=6r231">'
js = '<script src="/patch/sf-build-room-renderer.js?v=6r231"></script>'
block_head = '\n<!-- Signal Flow Build-a-Room consolidated renderer v6r231 -->\n' + css + '\n<!-- /Signal Flow Build-a-Room consolidated renderer v6r231 -->\n'
block_body = '\n<!-- Signal Flow Build-a-Room consolidated renderer v6r231 -->\n' + js + '\n<!-- /Signal Flow Build-a-Room consolidated renderer v6r231 -->\n'
if '</head>' not in html.lower() or '</body>' not in html.lower():
    raise SystemExit('Launch HTML is missing </head> or </body>')
html = re.sub(r'sf-build-room-v6r\d+-mount', 'sf-build-room-v6r231-mount', html)
...
out.write_text(json.dumps({'version':'6r231','shelves':shelves}, indent=2))
...
print('[OK] Installed Build-a-Room consolidated renderer v6r231')

...
(function () {
  ...
  function playGoodConnect() {
    // Disabled for now: correct Live Sound SFX assets/tones are not finalized.
    console.log("[Signal Flow] Native valid-connect sound skipped.");
  }

  function playBadConnect() {
    // Disabled for now: correct Live Sound SFX assets/tones are not finalized.
    console.log("[Signal Flow] Native invalid-connect sound skipped.");
  }
  ...
  function restoreNativeFrontPanel(attempt) {
    ...
    const ready = frontPanelLooksReady();
    const existingLayer = document.querySelector(".sf-live-native-layer");
    const surface = findSurface();

    console.log("[Signal Flow] Front restore attempt:", {
      attempt,
      ready,
      hasNativeLayer: !!existingLayer,
      hasSurface: !!surface,
      surfaceText: surface ? textOf(surface).slice(0, 100) : ""
    });

    if (ready) {
      if (existingLayer) existingLayer.remove();
      if (surface) restoreLegacyBoard(surface);

      setTimeout(() => {
        console.log("[Signal Flow] Front restore remount executing.");
        mountNative(true);
      }, 120);

      setTimeout(() => mountNative(false), 360);
      setTimeout(() => mountNative(false), 760);
      return;
    }
    ...
    console.warn("[Signal Flow] Front restore fallback reached.");

    if (existingLayer) existingLayer.remove();
    if (surface) restoreLegacyBoard(surface);

    setTimeout(() => mountNative(true), 120);
    setTimeout(() => mountNative(false), 360);
    setTimeout(() => mountNative(false), 760);
  }
  ...
  document.addEventListener("click", event => {
    ...
    if (label.includes("show hints") || label.includes("hide hints") || label === "hints") {
      // Let the original game handle the hint toggle, then lift the hint UI above
      // the native layer.
      setTimeout(raiseHintOverlays, 80);
      setTimeout(raiseHintOverlays, 250);
      setTimeout(raiseHintOverlays, 600);
      return;
    }

...
(function () {
  ...
  function findChecklistRow(routeKey) {
    const route = LEVEL.validRoutes.find(r => r.key === routeKey);
    if (!route) return null;

    const targets = route.checklist
      .toLowerCase()
      .split("→")
    ...
    const candidates = Array.from(document.querySelectorAll("div, li, button"))
      .filter(el => {
        ...
        if (el.closest(".sf-live-native-layer")) return false;

        const text = normalize(textOf(el));
        if (!targets.every(part => text.includes(part))) return false;
        if (!/(to do|complete)/.test(text)) return false;

        const r = el.getBoundingClientRect();

        // Keep this tightly scoped to the left task/checklist rows.
        // Avoid matching the whole sidebar or page shell.
        if (r.width < 160 || r.width > 420) return false;
        if (r.height < 34 || r.height > 115) return false;
  ...
  function markChecklist(routeKey) {
    const row = findChecklistRow(routeKey);
    if (!row) {
      console.warn("[Signal Flow] Native checklist row not found:", routeKey);
      return;
    }
    ...
    const badge = Array.from(row.querySelectorAll("span, div, b, strong"))
      .find(el => normalize(textOf(el)) === "to do" || normalize(textOf(el)).includes("to do"));
  ...
  function unmarkChecklist(routeKey) {
    const row = findChecklistRow(routeKey);
    if (!row) {
      console.warn("[Signal Flow] Native checklist row not found for unmark:", routeKey);
      return;
    }
    ...
    const badge = Array.from(row.querySelectorAll("span, div, b, strong"))
      .find(el => normalize(textOf(el)) === "complete" || normalize(textOf(el)).includes("complete"));

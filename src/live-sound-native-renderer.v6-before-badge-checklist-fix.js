...
(function () {
  ...
  function findChecklistRow(routeKey) {
    ...
    const all = Array.from(document.querySelectorAll("div, li, button"))
      .filter(el => {
    ...
    // Prefer actual route rows, not the tiny TO DO badge elements.
    const rows = all.filter(el => {
      const text = normalize(textOf(el));
      return (
        text.includes("→") ||
        text.includes("matrix") ||
        text.includes("main left") ||
        text.includes("main right") ||
        text.includes("lead vocal") ||
        text.includes("keys left") ||
        text.includes("keys right")
      );
    });

    return rows[routeIndex] || null;
  }
  ...
  function markChecklist(routeKey) {
    ...
    const badge = Array.from(row.querySelectorAll("span, div, b, strong, button"))

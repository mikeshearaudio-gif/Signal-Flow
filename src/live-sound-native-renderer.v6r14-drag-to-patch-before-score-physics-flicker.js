...
(function () {
  ...
  function updateNativePatchDrag(event) {
    ...
    const target = nativeNodeFromDocumentPoint(patchDrag.layer, event);
    const valid = target && target.key !== patchDrag.fromNode.key
      ? !!routeFor(patchDrag.fromNode.key, target.key)
      : false;

    const path = getDragPreviewPath(patchDrag.layer);
    path.setAttribute("d", cableD(patchDrag.fromNode.point, point, 0));

    if (target && target.key !== patchDrag.fromNode.key) {
      path.setAttribute("stroke", valid ? "#55e36f" : "#ff4f4f");
      path.style.filter = valid
        ? "drop-shadow(0 0 10px rgba(85,227,111,.56))"
        : "drop-shadow(0 0 10px rgba(255,79,79,.58))";
    } else {

...
(function () {
  ...
  function markChecklist(routeKey) {
    ...
    const item = candidates.find(el => {
      ...
      return targets.every(part => text.includes(part));
    });

    if (!item) return;

    const row = item.closest("li, div, button") || item;
    ...
    const badge = Array.from(row.querySelectorAll("span, div, b, strong"))
      .find(el => normalize(textOf(el)).includes("to do"));
  ...
  function unmarkChecklist(routeKey) {
    ...
    const badge = Array.from(row.querySelectorAll("span, div, b, strong"))
      .find(el => normalize(textOf(el)).includes("complete"));
  ...
  document.addEventListener("click", event => {
    ...
    if (label.includes("front panel") || label === "front") {
      ...
      nativeVisible = true;
      // Allow the original game button to perform the actual panel swap, then remount.
      setTimeout(() => mountNative(true), 160);
      setTimeout(() => mountNative(false), 500);
      setTimeout(() => mountNative(false), 1000);
      return;
    }
  }, true);

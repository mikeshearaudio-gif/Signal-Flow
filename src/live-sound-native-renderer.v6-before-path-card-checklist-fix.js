...
(function () {
  ...
  function findChecklistTarget(routeKey) {
    ...
    const badges = Array.from(document.querySelectorAll("span, div, b, strong, button"))
      .filter(el => {
        if (!el || !el.isConnected) return false;
        if (el.closest(".sf-live-native-layer")) return false;

        const text = normalize(textOf(el));
        if (text !== "to do" && text !== "complete") return false;
    ...
    for (let i = 0; i < 8 && row && row.parentElement; i++) {
      ...
      if (
        ...
        r.height >= 34 &&
        r.height <= 110 &&
        text.includes(normalize(textOf(badge))) &&
        (
    ...
    return { row: badge.parentElement || badge, badge };
  }

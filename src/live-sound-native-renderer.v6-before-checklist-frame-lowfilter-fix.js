...
(function () {
  ...
  function findChecklistTarget(routeKey) {
    ...
    for (const doc of checklistDocuments()) {
      const rows = Array.from(doc.querySelectorAll(".path-card"))
        .filter(row => {
          ...
          return /\b(to do|complete)\b/.test(text);
        })
        .sort((a, b) => a.getBoundingClientRect().top - b.getBoundingClientRect().top);

      fallbackRows = fallbackRows.concat(rows);
      ...
      if (exact) {
        const badge = exact.querySelector(".todo-badge");
        console.log("[Signal Flow] Native checklist target matched:", routeKey, textOf(exact));
        return { row: exact, badge };
      }
    }
    ...
    const fallback = fallbackRows[routeIndex];
    if (fallback) {
      const badge = fallback.querySelector(".todo-badge");
      console.log("[Signal Flow] Native checklist fallback matched:", routeKey, textOf(fallback));
      return { row: fallback, badge };
    }

    console.warn("[Signal Flow] Native checklist rows available:", fallbackRows.map(row => textOf(row)));
    return null;
  }

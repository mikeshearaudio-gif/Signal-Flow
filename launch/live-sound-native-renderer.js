...
function renderLiv009DrumStageInputs(surface, adapter) {
  ...
  function sfLiv010BuildPanels(width) {
    /*
      Locked LIV-010 vertical layout:
      FOH centered at top, 3-way crossover under it, then High/Mid/Low amps,
      then the PA visual below the amp stack with left/right line-array input
      panels flanking the PA image. This owns the geometry; do not use the
      generic right-column layout for this board.
    */
    const rackW = Math.max(620, Math.min(width * 0.80, 840));
    const rackX = Math.max(24, (width - rackW) / 2);

    const foh = { kind: "foh", x: rackX, y: 76, w: rackW };
    const crossover = {
      kind: "crossover",
      x: rackX,
      y: foh.y + rackW * (260 / 1120) + 52,
      w: rackW
    };
    const highAmp = {
      kind: "amp-high",
      x: rackX,
      y: crossover.y + rackW * (250 / 940) + 44,
      w: rackW
    };
    const midAmp = {
      kind: "amp-mid",
      x: rackX,
      y: highAmp.y + rackW * (240 / 940) + 38,
      w: rackW
    };
    const lowAmp = {
      kind: "amp-low",
      x: rackX,
      y: midAmp.y + rackW * (240 / 940) + 38,
      w: rackW
    };

    const paW = Math.max(330, Math.min(width * 0.38, 450));
    const sideW = Math.max(118, Math.min(width * 0.135, 156));
    const sideGap = Math.max(18, Math.min(width * 0.026, 30));
    const groupW = sideW + sideGap + paW + sideGap + sideW;
    const groupX = Math.max(20, (width - groupW) / 2);
    const paY = lowAmp.y + rackW * (240 / 940) + 68;
    const paVisual = { kind: "pa-visual", x: groupX + sideW + sideGap, y: paY, w: paW };
    const sideY = paY + Math.max(0, (paW * (870 / 857) - sideW * (620 / 260)) / 2);
    ...
    return {
      ...
      lowAmp,
      paVisual,
      speakerLeft: { kind: "speaker-left", x: groupX, y: sideY, w: sideW },
      speakerRight: { kind: "speaker-right", x: groupX + sideW + sideGap + paW + sideGap, y: sideY, w: sideW }
    };
  }

  function sfLiv010InstallStyle() {
    if(document.getElementById("sf-liv010-dedicated-style")) return;
    const style = document.createElement("style");
  ...
  function renderLiv010ThreeWayCrossover(surface, adapter) {
    ...
    surface.style.position = getComputedStyle(surface).position === "static" ? "relative" : surface.style.position;
    ...
    const boardHeight = Math.max(1900, Math.ceil(panels.paVisual.y + sfLiv010PanelHeight(panels.paVisual) + 120));

    const layer = document.createElement("div");
    layer.className = "sf-live-native-layer sf-live-native-level-liv-010";
    layer.style.cssText = [
      "position:absolute",
      "left:0",
      "top:0",
      ...
      "overflow:visible",
      "border-radius:18px",
      "background:radial-gradient(circle at 24% 12%, rgba(41,126,93,.13) 0%, rgba(41,126,93,.06) 22%, rgba(0,0,0,0) 46%), linear-gradient(180deg, rgba(2,8,9,.91), rgba(3,9,10,.95) 50%, rgba(3,8,9,.98))"
    ].join(";");

    createLabel(layer, "3-WAY CROSSOVER PA FEED - NATIVE CONCEPT MODE", 24, 20, 12);
    createLabel(layer, "FOH MAIN OUTPUTS", panels.foh.x + 12, panels.foh.y - 22, 11);
    createLabel(layer, "3-WAY CROSSOVER", panels.crossover.x + 12, panels.crossover.y - 22, 11);
    createLabel(layer, "HIGH AMP", panels.highAmp.x + 12, panels.highAmp.y - 22, 11);
    createLabel(layer, "MID AMP", panels.midAmp.x + 12, panels.midAmp.y - 22, 11);
    createLabel(layer, "LOW AMP", panels.lowAmp.x + 12, panels.lowAmp.y - 22, 11);
    createLabel(layer, "LINE ARRAY INPUTS", panels.speakerLeft.x, panels.speakerLeft.y - 22, 11);
    ...
    // Speaker input panels. The large PA artwork is visual-only.
    ...
    spacer.style.cssText = "position:relative;display:block;width:1px;min-height:" + boardHeight + "px;opacity:0;pointer-events:none";
    ...
    console.log("[Signal Flow] LIV-010 locked dedicated renderer mounted.");
  }
